...
// Read VUS from environment, default to 500
const VUS = parseInt(__ENV.VUS || '500');
const NORMAL_VUS = Math.floor(VUS * 0.1);  // 10% for normal load

// Spike test - sudden increase in load
export const options = {
  stages: [
    { duration: '30s', target: NORMAL_VUS },  // Normal load
    { duration: '10s', target: VUS },         // Sudden spike!
    { duration: '1m', target: VUS },          // Maintain spike
    { duration: '10s', target: NORMAL_VUS },  // Drop to normal
...
export function setup() {
  console.log('=== SPIKE TEST (CUSTOM) STARTING ===');
  console.log(`Target: ${BACKV4_URL} and ${BACKV5_URL}`);
  console.log(`VUS Configuration: Normal=${NORMAL_VUS}, Spike=${VUS}`);
