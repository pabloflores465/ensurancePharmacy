...
// Read VUS from environment, default to 100
const VUS = parseInt(__ENV.VUS || '100');
const STAGE1 = Math.floor(VUS * 0.5);  // 50% of target
const STAGE2 = VUS;                     // 100% of target
const STAGE3 = Math.floor(VUS * 2);    // 200% of target
const STAGE4 = Math.floor(VUS * 3);    // 300% of target

// Stress test configuration - push system beyond normal capacity
export const options = {
  stages: [
    { duration: '1m', target: STAGE1 },   // Ramp up to normal load
    { duration: '2m', target: STAGE2 },   // Increase to high load
    { duration: '2m', target: STAGE3 },   // Push to stress level
    { duration: '2m', target: STAGE4 },   // Maximum stress
...
export function setup() {
  console.log('=== STRESS TEST (CUSTOM) STARTING ===');
  console.log(`Target: ${BACKV4_URL} and ${BACKV5_URL}`);
  console.log(`VUS Configuration: Stage1=${STAGE1}, Stage2=${STAGE2}, Stage3=${STAGE3}, Stage4=${STAGE4}`);
