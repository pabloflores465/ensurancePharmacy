...
// Usar variables de entorno con fallbacks a configuración dinámica
const INSURANCE_API_URL =
  import.meta.env.VITE_ENSURANCE_API_URL || `http://${INSURANCE_IP}:8080/api`;
const HOSPITAL_API_URL = `http://localhost:8000`; // Usar localhost explícitamente para integración externa
const PHARMACY_API_URL =
  import.meta.env.VITE_PHARMACY_API_URL || `http://${PHARMACY_IP}:8081/api`;
...
const getApprovedHospitals = async (): Promise<Hospital[]> => {
  try {
    const response = await axios.get(
      `http://${INSURANCE_IP}:8080/api/hospital`
    );
    return response.data;
  } catch (error) {
...
const getHospitalServices = async (
  ...
  try {
    console.log(
      `Fetching services for hospital ID: ${hospitalId || "default"}`
    );
    ...
    if (!port) {
      ...
      if (
        defaultHospital &&
        defaultHospital.idHospital === hospitalId &&
        defaultHospital.port
      ) {
    ...
    try {
      ...
      console.log(
        "Alternative hospital services response:",
        alternativeResponse.data
      );
      if (alternativeResponse.data) {
        return Array.isArray(alternativeResponse.data)
          ? alternativeResponse.data
          : [alternativeResponse.data];
      }
    } catch (altErr) {
    ...
    console.warn(
      "No se pudo obtener servicios del hospital, devolviendo array vacío"
    );
    return [];
  } catch (error) {
...
const getMedicationsList = async () => {
  try {
    ...
    try {
      ...
      console.log(
        "Main medicines endpoint failed, trying alternative:",
        mainError
      );
...
const verifyMedicationCoverage = async (
  medicationId: string,
  insuranceId: string
) => {
  try {
    const response = await axios.get(
      `${INSURANCE_API_URL}/medication-coverage`,
      {
        params: {
          medicationId,
          insuranceId,
...
const requestServiceApproval = async (approvalData: ApprovalData) => {
  try {
    const response = await axios.post(
      `${INSURANCE_API_URL}/service-approvals/request`,
      approvalData
    );
    return response.data;
  } catch (error) {
...
const checkApprovalStatus = async (approvalCode: string) => {
  try {
    const response = await axios.get(
      `${INSURANCE_API_URL}/service-approvals/check/${approvalCode}`
    );
    return response.data;
  } catch (error) {
...
const syncHospitalServices = async (
  hospitalId: number,
  insuranceId: string,
  hospitalPort?: string
) => {
  try {
    ...
    const transformedServices = hospitalServices.map(
...
const registerApprovedService = async (serviceData: any) => {
  try {
    ...
    const response = await axios.post(
      `${INSURANCE_API_URL}/insurance-services/register`,
      transformedData
    );
    return response.data;
  } catch (error) {
...
const registerApprovedMedication = async (medicationData: any) => {
  try {
    ...
    const response = await axios.post(
      `${INSURANCE_API_URL}/pharmacy-medications/register`,
      transformedData
    );
    return response.data;
  } catch (error) {
