...
const primaryIP = import.meta.env.VITE_IP;
...
const HOSPITAL_API_URL = computed(() => `http://${primaryIP}:${hospitalPort.value}`);
...
async function tryMultipleIPs(endpoint: string, method: string = 'GET', data: any = null) {
  // Usar solo primaryIP en lugar de intentar múltiples IPs
  let lastError = null;
  
  try {
    console.log(`Intentando ${method} a http://${primaryIP}:${insurance_port}/api${endpoint}`);
    const url = `http://${primaryIP}:${insurance_port}/api${endpoint}`;
    ...
    if (!connectionIP.value) {
      connectionIP.value = primaryIP;
      console.log(`Conexión exitosa a ${primaryIP}`);
    }
    
    return response;
  } catch (error: any) {
    lastError = error;
    console.error(`Error con IP ${primaryIP}:`, error.message);
    
    if (lastError?.code === 'ECONNABORTED') {
      throw new Error(`Tiempo de espera agotado al conectar con el servidor. Verifique que el servidor esté ejecutándose.`);
    } else if (lastError?.response) {
      throw new Error(`El servidor respondió con un error: ${lastError.response.status} - ${lastError.response.statusText}`);
    } else if (lastError?.request) {
      throw new Error(`No se recibió respuesta del servidor. Verifique la conectividad de red y que el servidor esté ejecutándose.`);
    } else {
...
const onHospitalSelected = (port: string) => {
  ...
  // Load data after hospital selection
  Promise.all([fetchHospitalServices(), fetchCategories()]).catch(err => {
...
const fetchHospitalServices = async () => {
  try {
    ...
    // Obtener servicios directamente del hospital usando el puerto seleccionado
    const hospitalResponse = await axios.get(`${HOSPITAL_API_URL.value}/api/services`);
    
    if (hospitalResponse.data && hospitalResponse.data.services) {
      // Formatear los datos obtenidos del hospital
      const hospitalData = hospitalResponse.data.services.map((service: {
        _id: string,
        name: string,
        price?: number
      }) => ({
        hospitalServiceId: service._id,
        name: service.name,
        imported: false, // Por defecto asumimos que no están importados
        total: service.price || 0
      }));
      
      console.log("Servicios del hospital:", hospitalData);
      
      // Ahora verificamos cuáles ya están importados consultando a nuestra API
      try {
        const importedResponse = await tryMultipleIPs('/insurance-services/hospital-services', 'GET');
        const importedServices = importedResponse.data || [];
        
        // Marcar los servicios que ya están importados
        hospitalData.forEach((service: {
          hospitalServiceId: string,
          name: string,
          imported: boolean,
          insuranceServiceId?: number,
          ...
          const imported = importedServices.find((imp: any) => 
            imp.hospitalServiceId === service.hospitalServiceId
          );
          if (imported) {
            service.imported = true;
            service.insuranceServiceId = imported.insuranceServiceId;
          }
        });
        
        hospitalServices.value = hospitalData;
      } catch (importError) {
        console.error("Error al verificar servicios importados:", importError);
        hospitalServices.value = hospitalData; // Usamos los datos del hospital sin verificar importados
      }
    } else {
      hospitalServices.value = [];
      error.value = "No se pudo obtener servicios del hospital. Formato de respuesta inesperado.";
    }
  } catch (err) {
...
const testConnection = async () => {
  ...
  for (const ip of possibleIPs) {
    try {
      ...
      await axios.get(`http://${ip}:${insurance_port}/api/healthcheck`, {
        timeout: 3000
      });
      results[ip] = true;
    } catch (err) {
      try {
        ...
        await axios.get(`http://${ip}:${insurance_port}/api/users/count`, {
          timeout: 3000
        });
        results[ip] = true;
      } catch (err2) {
...
onMounted(async () => {
  // Datos se cargarán después de seleccionar el hospital
});
...
<template>
  <div>
    ...
    <div class="p-6" v-if="showContent">
