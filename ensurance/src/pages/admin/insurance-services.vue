...
const fetchCategories = async () => {
  try {
    const response = await axios.get(`http://${ip}:${insurance_port}/api/category`);
    if (response.data) {
  ...
  try {
    ...
    if (editMode.value && currentService.value.idInsuranceService) {
      ...
      response = await axios.put(
        `http://${ip}:${insurance_port}/api/insurance-services/${currentService.value.idInsuranceService}`,
        serviceData
      );
      ...
      response = await axios.post(
        `http://${ip}:${insurance_port}/api/insurance-services`,
        serviceData
      );
...
const toggleServiceStatus = async (service: InsuranceService) => {
  try {
    ...
    await axios.put(
      `http://${ip}:${insurance_port}/api/insurance-services/${service.idInsuranceService}`,
      updatedService
    );
...
const deleteService = async (service: InsuranceService) => {
  ...
  try {
    ...
    await axios.delete(`http://${ip}:${insurance_port}/api/insurance-services/${service.idInsuranceService}`);
