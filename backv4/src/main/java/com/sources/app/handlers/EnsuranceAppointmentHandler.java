 ...
 * relacionadas con la entidad {@link EnsuranceAppointment}, que representa
 * citas médicas posiblemente vinculadas a un sistema de seguros o un sistema
 * externo de hospital. Gestiona las solicitudes para el endpoint
 * "/api/ensurance-appointments".
 *
 * <p>
 * Endpoints manejados:</p>
 * <ul>
 * <li>{@code GET /api/ensurance-appointments}: Obtiene todas las citas.</li>
 * <li>{@code GET /api/ensurance-appointments?id={id}}: Obtiene una cita por su
 * ID primario (interno).</li>
 * <li>{@code GET /api/ensurance-appointments?hospitalId={hospitalId}}: Obtiene
 * una cita por el ID original del sistema de hospital.</li>
 * <li>{@code GET /api/ensurance-appointments?userId={userId}}: Obtiene todas
 * las citas para un usuario específico.</li>
 * <li>{@code GET /api/ensurance-appointments?today=true}: Obtiene todas las
 * citas programadas para el día actual.</li>
 * <li>{@code GET /api/ensurance-appointments?date={yyyy-MM-dd}}: Obtiene todas
 * las citas programadas para una fecha específica.</li>
 * <li>{@code POST /api/ensurance-appointments}: Crea una nueva cita.</li>
 * <li>{@code PUT /api/ensurance-appointments}: Actualiza una cita existente
 * (requiere 'id' en el cuerpo).</li>
 * <li>{@code DELETE /api/ensurance-appointments?hospitalId={hospitalId}}:
 * Elimina una cita usando el ID original del sistema de hospital.</li>
 * <li>(DELETE por ID interno podría implementarse como
 * {@code DELETE /api/ensurance-appointments/{id}}).</li>
 * </ul>
 */
public class EnsuranceAppointmentHandler implements HttpHandler {
    ...
    /**
     * Constructor del manejador de citas de seguro. Inicializa el DAO de citas
     * y el ObjectMapper, configurando un formato específico para la
     * serialización/deserialización de fechas y horas ("yyyy-MM-dd HH:mm:ss").
     *
     * @param appointmentDAO El DAO para interactuar con la tabla de citas
     * {@link EnsuranceAppointment}.
     */
    public EnsuranceAppointmentHandler(EnsuranceAppointmentDAO appointmentDAO) {
        this.appointmentDAO = appointmentDAO;
        this.objectMapper = new ObjectMapper().findAndRegisterModules();
        // Alinear formato de fecha con expectativas de tests (yyyy-MM-dd) y parseo estricto
     ...
     * solicitudes OPTIONS (preflight), y enruta las solicitudes GET, POST, PUT
     * y DELETE a sus respectivos métodos de manejo basados en el método HTTP,
     * la ruta y los parámetros de consulta. Captura excepciones generales para
     ...
     * manejado internamente y resultando en una respuesta de error 500).
     */
    @Override
    ...
    /**
     * Enruta las solicitudes GET a los manejadores específicos basados en los
     * parámetros de consulta. Determina qué tipo de búsqueda GET se solicita
     * (por hospitalId, fecha, userId, ID interno, o todas) y llama al método
     * correspondiente. Si no se reconoce ningún parámetro válido o no hay
     * parámetros, se asume que se solicitan todas las citas.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @param query La cadena de consulta de la URI (puede ser {@code null} o
     * vacía).
    ...
    /**
     * Maneja la solicitud GET para obtener todas las citas de
     * {@link EnsuranceAppointment}. Llama a
     * {@link EnsuranceAppointmentDAO#findAll()} y envía la lista resultante
     * como JSON.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @throws IOException Si ocurre un error al obtener los datos o al enviar
     * la respuesta.
     */
    ...
    /**
     * Maneja la solicitud GET para obtener una cita específica por su ID
     * primario (interno). Parsea el ID de la cadena, busca la cita usando
     * {@link EnsuranceAppointmentDAO#findById(Long)} y la envía como JSON si se
     * encuentra. Devuelve 404 si no se encuentra o 400 si el ID es inválido.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @param idString El ID de la cita (obtenido del parámetro de consulta
     * 'id') como {@link String}.
     * @throws IOException Si ocurre un error al enviar la respuesta.
     */
    ...
    /**
     * Maneja la solicitud GET para obtener todas las citas asociadas a un ID de
     * usuario específico. Parsea el ID de usuario, busca las citas usando
     * {@link EnsuranceAppointmentDAO#findByUserId(Long)} y envía la lista
     * resultante (puede estar vacía) como JSON. Devuelve 400 si el ID de
     * usuario es inválido. Nota: Actualmente no enriquece la respuesta con
     * detalles del usuario.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @param userIdString El ID del usuario (obtenido del parámetro de consulta
     * 'userId') como {@link String}.
     * @throws IOException Si ocurre un error al enviar la respuesta.
     */
    ...
    /**
     * Maneja la solicitud GET para obtener una cita específica usando el ID
     * original del sistema de hospital. Busca la cita usando
     * {@link EnsuranceAppointmentDAO#findByHospitalAppointmentId(String)} y la
     * envía como JSON si se encuentra. Devuelve 404 si no se encuentra.
     * Requiere que el parámetro 'hospitalId' no sea nulo ni vacío.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @param hospitalId El ID de la cita en el sistema original del hospital
     * (obtenido del parámetro de consulta 'hospitalId').
     * @throws IOException Si ocurre un error al enviar la respuesta.
     */
    ...
    /**
     * Maneja la solicitud POST a {@code /api/ensurance-appointments} para crear
     * una nueva cita. Lee el cuerpo JSON de la solicitud, que debe contener los
     * detalles de la cita (incluyendo potencialmente
     * {@code hospitalAppointmentId}, {@code userId}, {@code date},
     * {@code status}, etc.). Valida los campos requeridos (ej: `userId`,
     * `date`, `hospitalAppointmentId` si es necesario). Llama a
     * {@link EnsuranceAppointmentDAO#create(EnsuranceAppointment)} para guardar
     * la nueva cita. Responde con 201 (Created) y el objeto de la cita creada
     * si tiene éxito. Responde con 400 si faltan campos requeridos, si los
     * datos son inválidos (e.g., formato de fecha incorrecto) o si el JSON es
     * inválido. Responde con 409 (Conflict) si ya existe una cita con el mismo
     * `hospitalAppointmentId`. Responde con 500 si ocurre un error interno
     * durante la creación.
     *
     * @param exchange El objeto {@link HttpExchange}.
    ...
    private void handlePost(HttpExchange exchange) throws IOException {
        try {
            ...
            String safeDoctorName = newAppointmentData.getDoctorName() != null ? newAppointmentData.getDoctorName() : "";
            String safeReason = newAppointmentData.getReason() != null ? newAppointmentData.getReason() : "";
    ...
    /**
     * Maneja la solicitud PUT a {@code /api/ensurance-appointments} para
     * actualizar una cita existente. Lee el cuerpo JSON de la solicitud, que
     * *debe* contener el ID primario (`id`) de la cita a actualizar. Valida la
     * presencia del ID y otros campos opcionales que se puedan actualizar.
     * Llama a {@link EnsuranceAppointmentDAO#update(EnsuranceAppointment)} para
     * aplicar los cambios. Responde con 200 (OK) y el objeto de la cita
     * actualizada si tiene éxito. Responde con 400 si falta el ID, si los datos
     * son inválidos o si el JSON es inválido. Responde con 404 si no se
     * encuentra una cita con el ID proporcionado. Responde con 500 si ocurre un
     * error interno durante la actualización.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @throws IOException Si ocurre un error al leer el cuerpo de la solicitud
     * o al enviar la respuesta.
     ...
     * (Actualmente no invocado directamente por el enrutador principal, pero
     * podría ser usado si se implementa DELETE
     * /api/ensurance-appointments/{id}). Llama a
     * {@link EnsuranceAppointmentDAO#deleteById(Long)}. Responde con 204 (No
     * Content) si la eliminación es exitosa. Responde con 404 si no se
     * encuentra la cita. Responde con 500 si ocurre un error interno.
     *
     * @param exchange El objeto {@link HttpExchange}.
    ...
    /**
     * Maneja la solicitud DELETE para eliminar una cita usando el ID original
     * del sistema de hospital. Extrae el 'hospitalId' de la query string. Llama
     * a {@link EnsuranceAppointmentDAO#deleteByHospitalAppointmentId(String)}.
     * Responde con 204 (No Content) si la eliminación es exitosa (se encontró y
     * eliminó la cita). Responde con 404 si no se encontró ninguna cita con ese
     * 'hospitalId'. Responde con 400 si falta el parámetro 'hospitalId' o es
     * inválido. Responde con 500 si ocurre un error interno durante la
     * eliminación.
     *
     * @param exchange El objeto {@link HttpExchange}.
     ...
     * @param data El objeto a serializar (puede ser una lista, un objeto
     * simple, etc.).
     ...
     * cuerpo JSON con una clave "error" y el mensaje proporcionado.
     *
     * @param exchange El objeto {@link HttpExchange}.
    ...
    /**
     * Maneja la solicitud GET para obtener todas las citas programadas para la
     * fecha actual (hoy). Obtiene la fecha actual, la formatea como
     * "yyyy-MM-dd" y llama a
    ...
    /**
     * Maneja la solicitud GET para obtener todas las citas programadas para una
     * fecha específica. Valida que la fecha proporcionada en la query string
     * tenga el formato "yyyy-MM-dd". Llama a
     * {@link EnsuranceAppointmentDAO#findByDate(String)} con la fecha
     * formateada.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @param dateStr La fecha como String (obtenida del parámetro de consulta
    ...
    /**
     * Método auxiliar (marcado como TODO) para potencialmente enriquecer la
     * lista de citas con información adicional del usuario antes de enviarla
     * como respuesta. Actualmente, solo devuelve la lista original.
     *
