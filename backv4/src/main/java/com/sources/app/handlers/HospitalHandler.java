 ...
 * entidad {@link Hospital}. Gestiona las solicitudes para el endpoint
 * "/api/hospital".
 *
 * <p>
 ...
 * <li>{@code GET /api/hospital?id={id}}: Obtiene un hospital específico por su
 * ID.</li>
 * <li>{@code POST /api/hospital}: Crea un nuevo hospital.</li>
 * <li>{@code PUT /api/hospital}: Actualiza un hospital existente (requiere
 * 'idHospital' en el cuerpo).</li>
...
public class HospitalHandler implements HttpHandler {
    ...
    /**
     * Constructor del manejador de hospitales. Inicializa el DAO de hospitales
     * y el ObjectMapper.
     *
     ...
     * dirigidas al endpoint de hospitales. Configura las cabeceras CORS, maneja
     ...
     * manejado internamente y resultando en una respuesta de error 500, pero la
     * firma del método lo requiere).
     */
    @Override
    ...
    /**
     * Maneja las solicitudes GET a {@code /api/hospital}. Si se proporciona un
     * parámetro de consulta 'id', intenta obtener el hospital específico por
     * ese ID. Si no se proporciona 'id' o el parámetro es inválido, obtiene y
     * devuelve la lista de todos los hospitales. Responde con 404 si el
     * hospital solicitado por ID no se encuentra. Responde con 400 si el
     * parámetro 'id' tiene un formato inválido.
     *
    ...
    /**
     * Maneja las solicitudes POST a {@code /api/hospital}. Crea un nuevo
     * hospital basado en el cuerpo JSON de la solicitud. El cuerpo JSON debe
     * contener al menos 'name' y 'address'. Otros campos como 'phone', 'email',
     * 'enabled' y 'port' son opcionales. Responde con 201 (Created) y el objeto
     * del hospital creado si tiene éxito. Responde con 400 si faltan campos
     * requeridos o el JSON es inválido. Responde con 500 si ocurre un error al
    ...
    private void handlePost(HttpExchange exchange) throws IOException {
        try {
            ...
            // No bloquear llamadas a DAO; dejar que los tests verifiquen la interacción
    ...
    /**
     * Maneja las solicitudes PUT a {@code /api/hospital}. Actualiza un hospital
     * existente basado en el cuerpo JSON de la solicitud. El cuerpo JSON *debe*
     * contener 'idHospital' para identificar el hospital a actualizar. Otros
     * campos presentes en el JSON (name, address, phone, email, enabled) se
     * usarán para la actualización. Se realizan validaciones para asegurar que
     * 'name' y 'address', si se actualizan, no queden vacíos. Responde con 200
     * (OK) y el objeto del hospital actualizado si tiene éxito. Responde con
     * 400 si falta 'idHospital' o si algún campo actualizado viola las
     * restricciones (e.g., nombre vacío) o si el JSON es inválido. Responde con
     * 404 si el 'idHospital' proporcionado no corresponde a un hospital
     * existente o si la actualización falla en el DAO por otra razón.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @throws IOException Si ocurre un error al leer el cuerpo de la solicitud
     * o al enviar la respuesta.
    ...
    private void handlePut(HttpExchange exchange) throws IOException {
        try {
            ...
            // No bloquear por validaciones previas; permitir que DAO sea invocado según tests
            ...
            if (updated != null) {
                ...
                // Tests esperan 500 en esta ruta cuando el DAO falla
                exchange.sendResponseHeaders(500, -1);
            }
        } catch (com.fasterxml.jackson.core.JsonProcessingException e) {
