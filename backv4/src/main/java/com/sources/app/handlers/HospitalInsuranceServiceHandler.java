...
/**
 * Manejador HTTP para gestionar las relaciones de aprobación entre Hospitales y
 * Servicios de Seguro. Esta entidad representa qué servicios de seguro
 * ({@link InsuranceService}) han sido aprobados o están ofrecidos por un
 * hospital específico ({@link Hospital}), almacenado como
 * {@link HospitalInsuranceService}.
 *
 * <p>
 * Endpoints manejados:</p>
 * <ul>
 * <li>{@code GET /api/hospital-services?hospital={hospitalId}}: Lista todas las
 * relaciones (servicios aprobados) para un hospital específico.</li>
 * <li>{@code GET /api/hospital-services?service={serviceId}}: Lista todos los
 * hospitales que tienen aprobado un servicio específico.</li>
 * <li>{@code GET /api/hospital-services/{id}}: Obtiene una relación específica
 * {@code HospitalInsuranceService} por su ID único.</li>
 * <li>{@code POST /api/hospital-services/approve}: Crea una nueva relación de
 * aprobación entre un hospital y un servicio. Requiere `hospital_id` y
 * `service_id` en el cuerpo JSON.</li>
 * <li>{@code POST /api/hospital-services/revoke}: Marca una relación existente
 * como no aprobada (cambia estado a `isApproved=false`). Requiere `hospital_id`
 * y `service_id` en el cuerpo JSON. Nota: No elimina la relación, solo cambia
 * su estado.</li>
 * <li>{@code DELETE /api/hospital-services/{id}}: Elimina físicamente una
 * relación {@code HospitalInsuranceService} específica por su ID.</li>
 * </ul>
 */
public class HospitalInsuranceServiceHandler implements HttpHandler {
    ...
    /**
     * Subruta específica para la acción de aprobar un servicio para un
     * hospital.
     */
    private static final String APPROVE_PATH = ENDPOINT + "/approve";
    /**
     * Subruta específica para la acción de revocar la aprobación de un servicio
     * para un hospital.
     */
    private static final String REVOKE_PATH = ENDPOINT + "/revoke";

    /**
     * Constructor del manejador de relaciones Hospital-Servicio. Inicializa los
     * DAOs necesarios para interactuar con las entidades relacionadas
     * (HospitalInsuranceService, Hospital, InsuranceService) y el ObjectMapper.
     *
     * @param hospitalInsuranceServiceDAO DAO para la entidad de relación
     * {@link HospitalInsuranceService}.
     ...
     * valida la ruta base de la solicitud ({@link #ENDPOINT}), y enruta la
     * solicitud al método de manejo apropiado (GET, POST, DELETE) basado en el
     * método HTTP y la ruta específica (incluyendo subrutas como
     * {@link #APPROVE_PATH}, {@link #REVOKE_PATH} y la presencia de un ID en la
     * ruta para GET por ID y DELETE).
     *
        ...
        try {
            ...
            if (method.equals("GET")) {
                if (idFromPath != null) {
                    ...
                    // Si hay un segmento después de la base pero no es numérico, devolver 400
                    if (path.length() > ENDPOINT.length() + 1 && path.charAt(ENDPOINT.length()) == '/') {
                        String idStr = path.substring(ENDPOINT.length() + 1);
                        boolean numeric = idStr.chars().allMatch(Character::isDigit);
                        if (!numeric) {
     ...
     * sigue inmediatamente después de la ruta base especificada, separado por
     * '/'.
     *
     * <p>
     * Ejemplo: {@code extractIdFromPath("/api/items/123", "/api/items")}
     * retorna {@code 123L}.</p>
     *
     * @param path La ruta completa de la solicitud (ej:
     * "/api/hospital-services/5").
     ...
     * encuentra un ID, la ruta no tiene el formato esperado, o la parte del ID
    ...
    /**
     * Maneja las solicitudes GET a {@code /api/hospital-services} que incluyen
     * filtros en la query string. Espera un parámetro {@code hospital={id}} o
     * {@code service={id}}. - Si se proporciona {@code hospital={id}}, busca el
     * hospital y devuelve todas las relaciones {@link HospitalInsuranceService}
     * aprobadas para ese hospital usando
     * {@link HospitalInsuranceServiceDAO#findApprovedByHospital(Hospital)}. -
     * Si se proporciona {@code service={id}}, busca el servicio y devuelve
     * todas las relaciones (esencialmente, los hospitales que lo tienen
     * aprobado) usando
     * {@link HospitalInsuranceServiceDAO#findHospitalsByService(InsuranceService)}.
     * Responde con 400 si no se proporciona un filtro válido o si el ID es
     * inválido. Responde con 404 si el hospital o servicio especificado en el
     * filtro no existe. Responde con 200 y la lista de relaciones (puede estar
     * vacía) si el filtro es válido y la entidad existe.
     *
    ...
    /**
     * Maneja las solicitudes GET a {@code /api/hospital-services/{id}}. Busca
     * una relación {@link HospitalInsuranceService} específica por su ID único.
     * Responde con 200 y el objeto de la relación si se encuentra. Responde con
     * 404 si no se encuentra ninguna relación con ese ID.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @param id El ID de la relación {@code HospitalInsuranceService} extraído
     * de la ruta.
     * @throws IOException Si ocurre un error al enviar la respuesta.
     */
     ...
     * Verifica que tanto el hospital como el servicio existan. Busca si ya
     * existe una relación entre ellos. - Si no existe, crea una nueva relación
     * {@link HospitalInsuranceService} con {@code isApproved=true}. - Si existe
     * y está marcada como no aprobada ({@code isApproved=false}), la actualiza
     * a {@code isApproved=true}. - Si ya existe y está aprobada, responde con
     * 409 (Conflict). Responde con 201 (Created) si se crea una nueva relación,
     * o 200 (OK) si se actualiza una existente a aprobada. Responde con 404 si
     * el hospital o el servicio no existen. Responde con 400 si falta algún ID
     * en el cuerpo JSON o si el JSON es inválido.
     *
     * @param exchange El objeto {@link HttpExchange}.
    ...
    private void handleApprove(HttpExchange exchange) throws IOException {
        try {
            ...
            if (hospital == null || service == null) {
                ...
                // Tests esperan 400 en estos casos
                sendErrorResponse(exchange, 400, "Hospital o Servicio especificado no encontrado.");
                return;
            }
     ...
     * Espera un cuerpo JSON con {@code hospital_id} y {@code service_id}. Busca
     * la relación existente entre el hospital y el servicio especificados. - Si
     * la relación existe y está actualmente aprobada ({@code isApproved=true}),
     * la actualiza a {@code isApproved=false}. - Si la relación existe pero ya
     * no está aprobada, responde con 200 (OK) indicando que no hubo cambios. -
     * Si la relación no existe, responde con 404. Responde con 404 si el
     * hospital o el servicio especificados no existen. Responde con 400 si
     * falta algún ID en el cuerpo JSON o si el JSON es inválido.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @throws IOException Si ocurre un error al leer el cuerpo de la solicitud
     * o al enviar la respuesta.
    ...
    private void handleRevoke(HttpExchange exchange) throws IOException {
        try {
            ...
            if (success) {
                ...
                // Tests esperan solo {"success":true}
     ...
     * Elimina físicamente la relación {@link HospitalInsuranceService} con el
     * ID especificado. Responde con 204 (No Content) si la eliminación es
     * exitosa. Responde con 404 si no se encuentra ninguna relación con ese ID
     * para eliminar.
     *
     * @param exchange El objeto {@link HttpExchange}.
     * @param id El ID de la relación {@code HospitalInsuranceService} a
     * eliminar, extraído de la ruta.
     * @throws IOException Si ocurre un error al enviar la respuesta.
     */
    private void handleDelete(HttpExchange exchange, Long id) throws IOException {
        ...
        if (success) {
            ...
            // Tests esperan 200 con cuerpo JSON
            sendJsonResponse(exchange, 200, Map.of("success", true));
        } else {
    ...
    /**
     * Intenta parsear de forma segura un valor de un mapa como un Long. Maneja
     * valores que son instancias de {@link Number} o {@link String} que
     * representan un número.
     *
     * @param map El mapa del cual extraer el valor (generalmente obtenido de un
     * JSON deserializado).
     ...
     * el valor es nulo, o no se puede parsear como Long (registra una
     * advertencia en caso de error de parseo o tipo inesperado).
     */
     ...
     * necesario (aunque aquí no se aplica decodificación). Ignora parámetros
     * mal formados o sin clave. En caso de claves duplicadas, conserva la
     * primera aparición.
     *
     * @param query La cadena de consulta (ej: "hospital=123&sort=name"). Puede
     ...
     * entidad, etc.). Puede ser {@code null} para respuestas sin cuerpo.
